...
esp_err_t display_init(display_t *disp) {
    ...
    // Clear struct in a way you like
    disp->io = NULL;
    disp->panel = NULL;
    disp->touch = NULL;
    disp->touch_io = NULL;
    ...
    spi_bus_config_t buscfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = LCD_H_RES * 60 * sizeof(uint16_t), // keep your size
    };
    ESP_RETURN_ON_ERROR(spi_bus_initialize(LCD_HOST, &buscfg, 0), TAG, "spi_bus_initialize failed");

    // Panel IO handle and panel handle
    ...
    esp_lcd_panel_io_spi_config_t io_config = {
        ...
        .pclk_hz = 20 * 1000 * 1000, // 20 MHz, top is 40 MHz for ST7796
        .lcd_cmd_bits = 8,
        .lcd_param_bits = 8,
        .spi_mode = 0,
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        ...
        .rgb_endian = LCD_RGB_ENDIAN_RGB, // depends on wiring. Adjust if colors aren't red and white
        .bits_per_pixel = 16,
    };
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_invert_color(panel_handle, false), TAG, "invert color failed"); // set true if colors look inverted
    ESP_RETURN_ON_ERROR(esp_lcd_panel_disp_on_off(panel_handle, true), TAG, "disp_on_off failed");
    enable_backlight();

    // Quick test with a temporary display_t
    display_t tmp_disp = {
        .io = io_handle,
        .panel = panel_handle,
        .touch = NULL,
    ...
    fill_screen(&tmp_disp, 0xF800); // Red
    vTaskDelay(pdMS_TO_TICKS(300));
    fill_screen(&tmp_disp, 0xFFFF); // White
    vTaskDelay(pdMS_TO_TICKS(300));
    ...
    // Treat touch IO creation as non-fatal: log but continue
    esp_err_t tp_io_ret = esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)LCD_HOST, &tp_io_cfg, &tp_io);
    if (tp_io_ret != ESP_OK) {
        ESP_LOGE(TAG,
                 "esp_lcd_new_panel_io_spi for touch failed (err=0x%x). "
    ...
    if (tp_io != NULL) {
        ...
        if (tp_ret != ESP_OK) {
            ...
            // optional: free tp_io if touch failed
    ...
    // Store into display_t so the rest of the code can use it
    disp->io = io_handle;
